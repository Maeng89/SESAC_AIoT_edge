...
model_predict = tf.saved_model.load('./model/converea')
        ...
        try:
            ...
            while True:
                ...
                if frame is None:
                    ...
                    if cv2.getWindowProperty(window_title, cv2.WND_PROP_AUTOSIZE) >= 0:
                        ...
                        try : 
                            ...
                            print(pred)
                            growth_level = str(np.argmax(pred, axis=1))
                            pred_list.append(growth_level)
                ...
                if keyCode == 27 or keyCode == ord('q'):
                    ...
                    break
            # while loop finish
            upload_growth(counts.most_common(1)[0][0]) #  [('cherry', 4)] kust
...
if __name__ == "__main__":
    ...
    def upload_growth(growth_level):
        #doc_ref = db.collection(db_collection).document(db_id)
        doc_ref.update({'is_running': True, 'growth_level': firestore.ArrayUnion([growth_level])})
        print('data upload')
